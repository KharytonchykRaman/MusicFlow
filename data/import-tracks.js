...
const { MUSIC_FOLDER_PATH } = require('./FileManager');
...
async function downloadResource(url, fileName) {
  try {
    const localPath = path.join(FileManager.MUSIC_FOLDER_PATH, fileName);

    fs.mkdirSync(FileManager.MUSIC_FOLDER_PATH, { recursive: true });

    console.log(`Скачиваю: ${url} -> ${localPath}`);
...
const initDB = () => {
  ...
  tracks.forEach(tr => {
    try {
      const fileName = `${tr.artist}-${tr.name}`;
      downloadResource(tr.preview_url, fileName);

      tr.local_url = path.join(MUSIC_FOLDER_PATH, fileName);
    } catch (error) {
